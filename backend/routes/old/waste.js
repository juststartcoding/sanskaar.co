...
router.get('/requests', auth, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error fetching waste requests', error: error.message });
  }
});
...
router.patch('/requests/:id/status', auth(['admin']), async (req, res) => {
  try {
...
router.get('/track/:trackingId', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error tracking request', error: error.message });
  }
});
...
router.delete('/requests/:id', auth, async (req, res) => {
  try {
    ...
    if (request.status === 'collected') {
      return res.status(400).json({ message: 'Cannot delete collected requests' });
    }
